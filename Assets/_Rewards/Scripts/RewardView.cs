...
namespace Rewards
{
    internal class RewardView : MonoBehaviour
    {
        ...
        [field: SerializeField] public AwardPeriod CurrentAwardPeriod;
        ...
        {
            get => PlayerPrefs.GetInt($"{CurrentAwardPeriod}/{CurrentSlotInActiveKey}");
            set => PlayerPrefs.SetInt($"{CurrentAwardPeriod}/{CurrentSlotInActiveKey}", value);
        }
        ...
        public DateTime? TimeGetReward
        {
            get
            {
                string data = PlayerPrefs.GetString($"{CurrentAwardPeriod}/{TimeGetRewardKey}");
                return !string.IsNullOrEmpty(data) ? DateTime.Parse(data) : null;
            }
            set
            {
                if (value != null)
                    PlayerPrefs.SetString($"{CurrentAwardPeriod}/{TimeGetRewardKey}", value.ToString());
                else
                    PlayerPrefs.DeleteKey($"{CurrentAwardPeriod}/{TimeGetRewardKey}");
            }
        }
    }
}
